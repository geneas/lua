...
local have_bit32, bit32 = pcall(require, "bit32")
if have_bit32 then
	_band = bit32.band
	_bor = bit32.bor
	_bxor = bit32.bxor
...
local function _shift(m, count)
	...
	local digits = floor(count / digit_width)
	...
	if units > 0 then
		...
		if bitops then
			for i = #m, max(1, -digits - 1), -1 do
				sr = _bor(_bshl(sr, digit_bits), m[i])
				m[i] = _band(_bshr(sr, units), digit_mask)
			end
		else
	...
	if bitops then
		for i, dig in ipairs(m) do
			...
			if prod > digit_max then
				carry = _bshr(prod, digit_bits)
				prod = _band(prod, digit_mask)
			else
				carry = 0
			end
			m[i] = prod
...
local function _divmod(a, b)
	...
	local r0 = floor(digit_squared / (b[len] * digit_value + b[len - 1] + 1))
	...
	repeat
		...
		if q0 > digit_max then
			q = floor(q0 / digit_value)	-- use high digit
		elseif qpos > 1 then
			q = q0
...
local function _loadstr(r, s, radix)
	...
	for c in gmatch(num, "%x") do
		...
		if digit > 9 then digit = (digit + 9) % 16 end
...
local function powm(m1, m2, m3)	
	local modulus = type(m3) == "number" and _mpi(m3) or m3
	local negmod = modulus.negative
	
	modulus.negative = nil
	...
	if len == 1 and m1[1] == 1 then				-- 1^n or (-1)^n
		return negative and m3 < 0 and m1
			or  negative and sub(modulus, 1)
			or  negmod and sub(1, modulus)
			or  _mpi(1)
	end
...
local function extract(m, pos)
	...
	local data = _shift(_extract(m, floor(pos / digit_width), 3), pos % digit_width)
